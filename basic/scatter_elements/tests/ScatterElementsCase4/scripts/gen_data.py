...
def get_golden(data, indices, updates, axis=0,reduction='none'):
    ...
    res = res.numpy().astype(np.float32)
    return res
...
def gen_golden_data_simple():
    ...
    input_data = np.random.uniform(-1, 1, [32, 64, 64]).astype(np.float32)
    input_indices = np.random.uniform(0, 30, [32, 32, 32]).astype(np.int32)
    input_updates = np.random.uniform(-10, 10, [32, 32, 32]).astype(np.float32)
