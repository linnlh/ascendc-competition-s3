...
def get_golden(data, indices, updates, axis=0,reduction='none'):
    ...
    updates = updates.astype(np.int32)
    if reduction == "add":
...
def gen_golden_data_simple():
    ...
    input_data = np.random.uniform(-5, 5, [4, 8, 12]).astype(np.int32)
    input_indices = np.random.uniform(0, 4, [2, 4, 6]).astype(np.int32)
    input_updates = np.random.uniform(-10, 10, [2, 4, 6]).astype(np.int32)
