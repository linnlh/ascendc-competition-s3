...
OperatorDesc CreateOpDesc()
{
    ...
    aclDataType inputType = ACL_UINT8;

    std::vector<int64_t> shape2{16, 1};
    std::vector<int64_t> shape1{8, 16, 1, 32};
    std::vector<int64_t> shape3{8, 16, 16, 32};
    OperatorDesc opDesc;
    opDesc.rtol = 0.01;
    opDesc.atol = 2.5;
    opDesc.equal_nan = false;
    opDesc.AddInputTensorDesc(inputType, shape1.size(), shape1.data(), format);
    opDesc.AddInputTensorDesc(inputType, shape2.size(), shape2.data(), format);

    opDesc.AddOutputTensorDesc(ACL_BOOL, shape3.size(), shape3.data(), format);
    return opDesc;
}
